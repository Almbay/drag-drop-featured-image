	...
	*/
	...
	function dgd_removeDefaultScripts(){
		...
		if (in_array($currentPostType, $selected)){
			...
			wp_register_style('dgd_uploaderStyle', $pluginDir.'drag-and-featured.css', false);
	...
	function dgd_addNewMetaBox(){
		$selected = get_option('drag-drop-post-types');
		foreach ($selected as $post_type){
			add_meta_box('drag_to_upload', __('Featured Image'), 'dgd_upload_meta_box', $post_type, 'side', 'high');
		}
	}
	
	// Add menu page to appearence:
	function dgd_setMenuPage(){
		add_theme_page('Options for Drag & Drop Featured Image', __('Drag to Feature'), 'manage_options', 'drag-to-feature', 'dragFeatureAdminPanel');
	}
	...
	function dgd_upload_meta_box(){ ?>
		...
		<div id="uploadContainer" style="margin-top: 10px;">
			...
			<div id="uploaderSection" style="position: relative;">
				<div class="loading"><img src="<?php echo $pluginDir; ?>loading.gif" alt="Loading..." /></div>
		...
		<?php
			...
			$plupload_init = array(
				...
				'filters'             => array(array('title' => __('Allowed Files'), 'extensions' => implode(',', get_option('drag-drop-file-types')))),
				'multipart'           => true,
	...
	function dragFeatureAdminPanel($info){ ?>
		<div class="wrap">
			...
			<h2><?php _e('Plugin options for drag to feature'); ?></h2>
