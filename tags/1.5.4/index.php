...
*/
	...
	add_action('admin_head', 'dgd_printPostID');
	...
	add_action('admin_enqueue_scripts', 'dgd_loadPanelScripts');
	add_action('admin_enqueue_scripts', 'dgd_removeDefaultScripts');
	add_action('wp_ajax_dgd_set_featured_image', 'dgd_ajaxPhotoUpload');
	add_action('wp_ajax_get_featured_image', 'dgd_getNewFeaturedImage');
	...
	// Print post ID in head:
	function dgd_printPostID(){
		global $post, $current_screen;
		if ($current_screen->base === 'post'){
			echo '<script type="text/javascript">var dgd_post_id = '.$post->ID.';</script>';
		}
	}
	...
	function dgd_removeDefaultScripts($hook){
	...
	function dgd_loadPanelScripts($hook){
		
		// Set plugin dir:
		$pluginDir = plugins_url('drag-drop-featured-image').'/';
		
		// Post edit screen:
		if ($hook === 'post.php' || $hook === 'post-new.php'){
			
			// Stylesheets:
			wp_register_style('dgd_uploaderStyle', $pluginDir.'includes/style/drag-drop-uploader.css', false);
			wp_enqueue_style('dgd_uploaderStyle');
			
			// Scripts:
			wp_register_script('dgd_uploaderScript', $pluginDir.'includes/scripts/drag-drop-uploader.js', 'jquery');
			wp_enqueue_script('dgd_uploaderScript');
			
		}
		
		// Plugin options screen:
		if ($hook === 'drag-drop-featured-image/drag-to-feature.php'){
			
			// Stylesheets:
			wp_register_style('itoggle_style', $pluginDir.'includes/style/engage.itoggle.css', false);
			wp_register_style('dgd_panelStyle', $pluginDir.'includes/style/drag-to-feature.css', false);
	...
	function dgd_setMenuPage(){
		$required_capability = get_option('drag-drop-user-capability') ? get_option('drag-drop-user-capability') : 'manage_options';
		add_options_page(__('Options for Drag & Drop Featured Image', 'dgdfi'), __('D&D Featured Image', 'dgdfi'), $required_capability, 'drag-drop-featured-image/drag-to-feature.php', '');
	}
	...
	function dgd_upload_meta_box(){ ?>
		<?php $pluginDir = plugins_url('drag-drop-featured-image').'/'; ?>
					...
					<div id="drag-drop-area">
						<div class="drag-drop-inside">
							...
							<p class="drag-drop-buttons">
								<?php global $wp_version; $currentVersion = substr(str_replace('.', '', $wp_version), 0, 2); ?>
								<?php if ($currentVersion >= 35): ?>
									<a href="#" id="dgd_library_button" class="button insert-media add_media" data-editor="content" title="Add Media"><span class="wp-media-buttons-icon"></span><?php _e('Media Library', 'dgdfi'); ?></a>
								<?php else: ?>
		...
		<?php
			...
			$plupload_init = array(
				...
				'multipart_params'    => array(
					...
					'action'      => 'dgd_set_featured_image', // The AJAX action name
					'postID'	  => $post->ID
				),
			);
	...
	function dgd_ajaxPhotoUpload(){
		...
		$status = wp_handle_upload($file, array('test_form'=>true, 'action' => 'dgd_set_featured_image'));
	...
	function dgd_getNewFeaturedImage(){
		global $post;
		...
		if ($postID){
			...
			$response = array(
				'response_code' => 500,
				'response_content' => __('Drag & Drop Feaured image: An error occured, please reload page!', 'dgdfi')
			);
		}
