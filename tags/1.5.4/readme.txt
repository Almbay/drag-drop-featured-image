...
To select a previously uploaded image from the Media Library, start by pressing the media buttons on top of the editor and go to the Media Library tab. Once there, expand the image you want to use and press "Set featured image" just as you would without the plugin. In versions 3.4.x and earlier the page needs to reload in order to properly display the selected image. This behavior is now handled by AJAX in Wordpress 3.5+.
